...
extends CustomHttpService[Future[JValue], (Token, Path) => Future[HttpResponse[JValue]]] {
  val service = (request: HttpRequest[Future[JValue]]) => {
    ...
    Success(
      (token: Token, path: Path) => {
        val order:  Int = request.parameters.get('order).flatMap(_.parseInt.toOption).getOrElse(token.limits.order)
              ...
              Future(
                fields flatMap { 
                  case JField(eventName, jvalue) => { 
                    ...
                    val trackingPath = basePath(path)

                    if (reprocess) {
                      if(trackableEvent) storageReporting.stored(trackingPath, 1)
  ...
  private def basePath(path: Path): Path = path.parent match {
    case Some(parent) if parent.equals(Path.Root) => path
    case Some(parent)                             => basePath(parent)
    case None                                     => error("Traversal to parent of root path should never occur.")
  }
